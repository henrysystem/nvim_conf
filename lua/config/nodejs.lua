...
local function get_system_node()
  ...
  local system_paths = {
    "C:/Program Files/nodejs/node.exe", -- Windows standard install
    "C:/Program Files (x86)/nodejs/node.exe", -- Windows x86 install
    vim.fn.expand("~/AppData/Local/Volta/bin/node.exe"), -- Volta on Windows
    vim.fn.expand("~/scoop/apps/nodejs/current/node.exe"), -- Scoop on Windows
    "C:/ProgramData/chocolatey/bin/node.exe", -- Chocolatey shim
  ...
  return vim.fn.exepath("node.exe")
end
    ...
    if vim.v.shell_error == 0 then
      ...
      if version then
        ...
        if major_version and major_version >= 18 then
          ...
          if node_host == "" then
            node_host = vim.fn.exepath("neovim-node-host.cmd")
          end

          if vim.fn.has("win32") == 1 and node_host:lower():match("%.cmd$") then
            node_host = ""
          end

          if node_host == "" then
            local npm_global_host = vim.fn.expand("~/AppData/Roaming/npm/node_modules/neovim/bin/cli.js")
            if vim.uv.fs_stat(npm_global_host) ~= nil then
              node_host = npm_global_host
            end
          end
